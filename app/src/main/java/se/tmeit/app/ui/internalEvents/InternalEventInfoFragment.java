...
public final class InternalEventInfoFragment extends Fragment implements MainActivity.HasTitle, MainActivity.HasNavigationItem {
	...
	private final ReportButtonClickListener mReportClickedListener = new ReportButtonClickListener();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		final View view = inflater.inflate(R.layout.fragment_internal_event_info, container, false);

		final Bundle args = getArguments();
		final TextView titleText = (TextView) view.findViewById(R.id.event_title);
		titleText.setText(args.getString(InternalEvent.Keys.TITLE));

		final TextView startDateText = (TextView) view.findViewById(R.id.event_start_date);
		...
		final TextView teamText = (TextView) view.findViewById(R.id.event_team);
		...
		mReportButton = (Button) view.findViewById(R.id.event_button_report);
	...
	private void beginLoad(boolean noCache) {
		...
		final String username = mPrefs.getAuthenticatedUserName();
		final String serviceAuth = mPrefs.getServiceAuthentication();
	...
	private void finishLoad(InternalEvent.RepositoryData repositoryData) {
		...
		final LinearLayout yesList = (LinearLayout) view.findViewById(R.id.event_workers_yes);
		final LinearLayout maybeList = (LinearLayout) view.findViewById(R.id.event_workers_maybe);
		final LinearLayout noList = (LinearLayout) view.findViewById(R.id.event_workers_no);
		...
		final List<InternalEventWorker> maybeWorkers = InternalEventWorker.filterByWorking(repositoryData.getWorkers(), InternalEventWorker.Working.MAYBE);
		final List<InternalEventWorker> noWorkers = InternalEventWorker.filterByWorking(repositoryData.getWorkers(), InternalEventWorker.Working.NO);
		...
		final InternalEvent event = repositoryData.getEvent();
		if (event.mayEditReport()) {
		...
		mReportButton.setOnClickListener(mReportClickedListener);
	...
	private void initializeListOfWorkers(View listLayout, LinearLayout listView, List<InternalEventWorker> workers) {
		...
		for (InternalEventWorker worker : workers) {
			final View view = layoutInflater.inflate(R.layout.list_item_internal_event_worker, listView, false);

			final TextView nameText = (TextView) view.findViewById(R.id.event_worker_name);
			...
			final TextView commentText = (TextView) view.findViewById(R.id.event_worker_comment);
			commentText.setText(TextUtils.isEmpty(worker.comment()) ? "-" : worker.comment());

			final TextView rangeTextView = (TextView) view.findViewById(R.id.event_worker_range_text);
			...
			if (worker.hasRange()) {
				...
				final int rangeStart = worker.rangeStart() % 24;
				final int rangeEnd = worker.rangeEnd() % 24;
	...
	private final class ReportButtonClickListener implements View.OnClickListener {
		@Override
